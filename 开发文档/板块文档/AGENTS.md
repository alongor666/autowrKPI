...
2) 再对照筛选器系统：确认维度 key、筛选状态、级联锁定是否会影响本板块展示与计算。参考 06_筛选器系统.md。  
...
依据：06_筛选器系统详细文档（filterState、applyDrillFilters、级联锁定等）。  
