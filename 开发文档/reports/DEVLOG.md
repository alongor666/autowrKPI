...
### 🐛 Bug修复：修复下钻筛选功能失效问题
**摘要**: 修复了“是否新能源”、“续保状态”、“终端来源”等新维度筛选无效，以及“应用筛选”按钮无响应的严重Bug。

**问题背景**:
- 用户选择新维度（如“是否新能源”）后点击“应用筛选”，数据未发生变化。
- 浏览器控制台报错 `this.applyFilters is not a function`。
- 原因：Worker 中缺失新维度的字段映射，Dashboard 中缺失 `applyFilters` 方法定义。

**主要变更**:
1.  **Worker 维度配置补全**
    - 在 `js/data.worker.js` 中补全了 `dimensionConfigMap`，添加了 `energy_type`, `renewal_status`, `terminal_source` 的映射。
2.  **Dashboard 方法补全**
    - 在 `js/dashboard.js` 中实现了 `applyFilters` 方法，负责收集筛选状态并发送给 Worker。

**影响文件**:
- `js/data.worker.js` [UPDATED]
- `js/dashboard.js` [UPDATED]

**验证方式**:
- 启动服务，上传数据。
- 筛选“是否新能源”为“是”，点击应用。
- 确认 KPI 和图表数据已正确过滤（仅显示新能源车数据）。

---

## 2025-12-17

### 🎨 UI/UX 优化：图表样式与交互全面升级
**摘要**: 全面优化了图表展示效果，使其符合 PPT 汇报标准，并增强了损失暴露分析和费用支出分析功能。

**主要变更**:
1.  **取消固定宽高比**
    - CSS 中移除 16:9 限制，改为 `min-height: 100vh`，自适应屏幕。
2.  **添加智能阈值线**
    - 所有核心图表增加动态阈值线（如变动成本率 94% 危险线）。
    - 样式优化为实线/虚线结合，标签清晰可见。
3.  **实现高级分析图表**
    - **损失暴露**: 新增“赔付VS占比”气泡图和“频度VS额度”象限图。
    - **费用支出**: 新增费用率分布柱状图，支持根据阈值自动变色（红/黄/绿）。
4.  **全局样式微调**
    - 移除所有图表网格线，保持画面整洁。
    - 字体全面加粗，提升可读性。
    - X轴标签优化：取消倾斜，自动截断长文本，防止重叠。
5.  **性能优化**
    - 引入防抖（Debounce）机制优化渲染性能。
    - 增加气泡大小计算缓存。

**影响文件**:
- `css/dashboard.css` [UPDATED]
- `js/dashboard.js` [UPDATED]
- `index.html` [UPDATED]

**验证方式**:
- 视觉检查：确认图表无网格线，字体加粗。
- 功能检查：切换损失暴露的两个子标签，确认图表正确切换。
- 交互检查：Hover 提示框显示正确数据，阈值线显示正确数值。
...
### 🐛 启动故障修复：元数据渲染缺失方法 (2025-12-21)

**摘要**: 启动后控制台报错 `this.addIntervalModeToggle is not a function`，导致渲染流程中断。

**问题现象**:
- 触发点: `dashboard.js` 中 `renderMetadata()` 调用 `addIntervalModeToggle`
- 报错影响: 页面初始化中断，仪表盘未完成渲染

**根因分析**:
- 新增的“区间模式切换器”调用已写入，但对应方法未实现或未合入

**修复方案**:
- 添加 `addIntervalModeToggle` 的安全空实现
- 在缺少 UI 容器时直接返回，避免影响现有渲染流程

**待办清单**:
1. 新增 `addIntervalModeToggle` 方法并保持 KISS
2. 确认无 UI 容器时不产生副作用
3. 验证页面初始化流程恢复
...
### 🐛 业务类型映射修正：非营业货车新旧标签 (2025-12-21)

**摘要**: 调整“非营业货车新车/旧车”的 UI 简称，避免与吨位档混淆。

**问题现象**:
- CSV 原始值为“非营业货车新车/旧车”时，UI 显示为“非营货-1–2t”
- 用户需要区分新车与旧车

**修复方案**:
- `reference/business_type_mapping.json` 中修正兼容映射的 `ui_short_label`
- `processBusinessMapping` 支持兼容映射自定义 `ui_short_label`

**待办清单**:
1. 更新兼容映射：新车→非营货-新，旧车→非营货-旧
2. 保持 canonical 类别与映射口径不变
3. 验证图表与筛选器显示新旧标签

**验证方式**:
```bash
python3 -m http.server 8888
# 导入包含“非营业货车新车/旧车”的CSV
# 业务类型图表与筛选器显示“非营货-新/非营货-旧”
```
