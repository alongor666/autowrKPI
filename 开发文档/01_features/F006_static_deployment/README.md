...
### 2025-12-15 (v3) - 最新需求对齐（概览/进度简化与标签规则）

1. **经营概览图表简化**
   - 不再展示保费计划达成率相关图形
   - 仅展示**客户类别**、**业务类型**的**变动成本率柱状图**
   - 变动成本率基准线固定为 **90%**

2. **保费进度图表简化**
   - 不再展示保费计划达成率相关图形
   - 仅展示**客户类别**、**业务类型**的**签单保费柱状图**

3. **气泡图标签规则调整**
   - 固定数字标签：文字统一为黑色，仅展示**变动成本率**
   - 动态标签（强调态）：当**正向指标占比 < 逆向指标占比**时，仅将**逆向指标占比数值**标红

### 2025-12-15 (v2) - UI增强与标签优化

**气泡图动态标签系统**：
1. **变动成本气泡图**
   - 新标签格式：`【机构名】 赔付率% | 费用率% | 成本率%`
   - 危险色判断：当成本率超阈值时标红
   - 实现加粗机构名，提升可读性
...
3. **赔付VS占比气泡图**
   - 新标签格式：`【机构名】 赔付率% | 赔款占比% | 保费占比%`
   - 危险色判断：当赔款占比 > 保费占比时标红（逆向指标超正向指标）

4. **频度VS额度气泡图**
   - 新标签格式：`【机构名】 出险率% | 案均¥ | 赔付率%`
   - 危险色判断：当出险率或案均赔款超阈值时标红

5. **费用支出气泡图**
   - 新标签格式：`【机构名】 费用率% | 费用占比% | 保费占比%`
   - 危险色判断：当费用占比 > 保费占比时标红

**技术实现**：
- 使用 ECharts rich text 实现机构名加粗 `{b|name}`
- 动态 `color` 和 `textBorderColor` 函数实现危险色判断
- 正常色：白色文字 + 黑色描边
- 危险色：#d32f2f 文字 + #8b0000 描边

### 2025-12-15 (v1) - 元数据提取与分析模式
